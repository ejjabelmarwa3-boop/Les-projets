...
public class UserDAO {
    // CONFIGURATION XAMPP
    private String jdbcURL = "jdbc:mysql://localhost:3306/demo_db?useSSL=false&serverTimezone=UTC&allowPublicKeyRetrieval=true";
    ...
    public User selectUser(int id) throws SQLException {
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    user = new User(rs.getInt("id"), rs.getString("nom"), rs.getString("email"));
                }
            }
        }
        return user;
